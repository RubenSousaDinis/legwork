...
## 15. Round 2+
Round 2 (Sept 6, #90 merged): every read joins `tasks` and keeps completed ones only (`completedTaskFilter`: `released`, or `resolved` to the worker); `syncObservations` reconciles the newest `SYNC_BATCH` completed tasks rather than filling gaps, so T-17's submit-time row is rewritten. §8's "a second call inserts none" means one row at the table, never a duplicate. `seeded = workerSeeded || task.seeded`. The contract's `publicObservations` now matches the route (`delta {checked_places, wrong_listings, by_source, sentence}`, `worker_verified`, `disclosure`, `include_seeded`). A `tasks.resolved_to` column is on the schema list; moving the reconcile behind `/admin/sweep` is a follow-up if it shows in latency.
