...
|---|---|---|
| `LiveDashboard { initial; taskId?; children: (data) => ReactNode }`, `useLiveDashboard`, `getDashboardData(mode, { taskId?, state? })` | T-26 `lib/live/*`, `lib/data/index.ts` | 3-s poll with the `changed` short-circuit; `?task=` pin |
