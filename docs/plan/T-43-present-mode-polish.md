...
## 15. Round 2+
Merged (Sept 6, #98): T-39's gate is green on `main`. Rulings — `data-row` lives on the wrapper `div` and **not** as a second `data-testid="task-row"` (a duplicated testid would count six rows and break both `hideCardsKeepsCentreFixed` and the gate), so §6's selector is `[data-row="n"] [data-testid="task-row"]`; present mode shows **one** screening line, the marked refusal, without its hash and mark reference (the right column's arithmetic left no room for a second at the 32-px floor); the three columns are placed in their grid tracks by name because a `?hide=` that empties the left column let the centre auto-place into track 1 and slid the meter out of the 9:16 band. The `e2e-dashboard` CI job was wired by the lead after this merge. §11's rehearsal-release check with `?crop=1` is the operator's.

**Required fixes from T-39's gate (Sept 6, #96) — `pnpm --filter @legwork/dashboard e2e` must pass (`presentModeLegible` and `cropBandMath`) before `gh pr ready`, and the gate itself (`apps/dashboard/e2e/**`) is T-39's: fix the CSS, never the floor, the viewport or the test.** Measured at 1920×1080, `deviceScaleFactor: 1`, `DATA_MODE=demo`, real Archivo / Inter / JetBrains Mono:
1. `.present-clock` (`15:53:19`) and `.present-elapsed` (`t+04:12 since posted`) sit at `box.top = −6.5` in both beats — `.present-header` is `justify-content: space-between` with a 40 px gap, `.present-time` is squeezed to 286 px, and the 24 px mono timer wraps to three lines inside a `calc(96 * var(--u))` header. Give the time group the width it needs (`white-space: nowrap` on `.present-elapsed`, `flex: 0 0 auto` on `.present-time`) or make the header tall enough; the 24 px floor itself holds.
2. `.stage .meter-tail` (`→ worker · +0.45 fee`, 32 px JetBrains Mono, content box ≈ 39 px) overflows the `line-height: 1.1` box of `.meter-line` (35.2 px) in the released beat — `line-height` ≥ 1.22 on `.meter-line` (or the tail on its own line). This is half of the money line, rule (10).
3. The pool chip `1 real · +20 seeded (demo data)` is 625 px wide at its 32 px floor inside a 538 px Supply-card content box and is cut to `1 real · +20 seeded (demo da` by `.card { overflow: hidden }` — rule (9) and the one string 05-demo-video names for the arm's-length read. Let the chip wrap, widen the card, or cut a card; never shorten the string or lower the floor.
4. The screening log's refusal line (`authentication circumvention · asks the worker…`) runs 162 px past `.screening.card` and the two `PASSED` lines below it never render (620 px, 771 px outside) — fewer lines in present mode, a taller card, or a card cut.
Also from the gate: add `data-column="left|centre|right"` on the three columns (already in §2) so the gate's primary selector replaces its fallback.
