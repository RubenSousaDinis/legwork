...
- **Rate limit** `apps/api/src/http/rateLimit.ts`: `rateLimit(key, {limit, windowS})` in-memory sliding window, throws `rate_limited` with `retry_after_seconds`; `clientKey(req)` = `x-forwarded-for` first hop or `'local'`. Per instance on Vercel — the README says so.
