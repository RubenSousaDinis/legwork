...
- Identifier regexes: PT licence plate (`\d{2}-[A-Z]{2}-\d{2}`, `[A-Z]{2}-\d{2}-\d{2}`, `\d{2}-\d{2}-[A-Z]{2}`, `[A-Z]{2}-\d{2}-[A-Z]{2}`) → `ident.plate`; a phone number in any leaf other than `spec.phone` and `spec.place.place_id` (an OSM id's digits are not a phone number), bounded like every other rule — `(?<![\p{L}\p{N}])(?:(?:\+?351)?\s?9\d{2}\s?\d{3}\s?\d{3}|\+\d{9,14})(?![\p{L}\p{N}])`; unbounded, a nine-digit run inside a `sha256` leaf reads as a phone and an honest compare-two is refused and marked → `ident.phone`; email → `ident.email`; `NIF|NISS|\bBI\b|cartão de cidadão` followed by digits → `ident.document`.
...
2. `levenshtein.ts`, `place-index.ts`, `leiria-min.json`; unit-test `fuzzyMatch` on Farmácia Central (`"Farmacia Central"` distance 0 — §2.2's normalisation strips the diacritic first; `"Farmacia Centrl"` is distance 1 — ok) and on `"Casa do João Silva"` (not ok).
