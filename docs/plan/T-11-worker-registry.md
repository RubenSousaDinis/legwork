...
3. Write `WorkerRegistryTest.setUp`: deploy with `Keys.deployer` as owner, `Keys.relayer`, `Keys.verifier`; `vm.chainId(84532)`; `vm.warp(1_757_000_000)`; a default attestation for `(nullifier = 0xA11CE, worker1, "ez1dp", 15, deadline = block.timestamp + 600)` signed by the verifier key.
