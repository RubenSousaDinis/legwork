...
| `earningsEarnedOnly` | scenario `zero` → `0.00` and `completed 0`; scenario `one_paid` (`released_usdc: 3, completed: 1, score: 5, distinct_raters: 1`) → `3.00`, `completed 1`; the page contains no figure that is not in the response (assert `4.6` and `11` absent) |
