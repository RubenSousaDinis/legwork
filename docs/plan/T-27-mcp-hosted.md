...
5. `test/fixtures/preflight-studio.json`: a recorded Studio response pair for `task_type: 'verify-open'`, `area: 'ez1dp'`, `now` fixed. Workers: one real (`seeded:false`, `taskTypes: 3`, completed 2 days ago), three seeded active, one seeded inactive (20 days), one real in another area. Variant A: three seeded released verify-open tasks (6, 9, 14 min) + one real released verify-open task (11 min). Variant B: same, but the real worker's released task is `photo-of` (they stay active; `n_real` for verify-open is 0).
...
## 15. Round 2+
Post-merge (Sept 6, wave-3 sync): `HireHumanInput` is now one object schema with `spec` as the four per-type shapes and a refinement that runs `Envelope`, so `tools/list` advertises every field; `approve_task` / `dispute_task` may answer `{ error: 'buyer_token_required', task_id, dashboard_url }` (`BuyerTokenRequired`) — the tools still answer with `isError` text until a follow-up switches them; `score_floor` is documented as `min(score)` over verified active workers (seeded fallback, then 0); `app/public/preflight/route.ts` calls `preflightWorkers`. `DEFAULT_API_BASE` / `DEFAULT_DASHBOARD_URL` wait for the deploy.
