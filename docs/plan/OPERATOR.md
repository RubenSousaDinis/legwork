...
- [x] Email `developers@toolsforhumanity.com` for Selfie Check (Beta) access — the longest-lead item. **Sent Sept 8** with the refusal payload, the app id and the RP id. It was needed: the credential is access-gated, the flag was never on, and the app gets `verification_disabled` until it is. No answer yet; the demo ships Orb meanwhile.
- [ ] Discord questions (verbatim texts in 07 item 3): ETHGlobal (phone screen recording; file vs URL), World (**answered by the Sept 8 run: an Orb credential verifies end to end through IDKit 4.x; Selfie Check cannot be exercised at all without the feature flag**), The Graph (Studio query URL as "live data"? Subgraph MCP as composing?).
...
- [x] Pick the shop — **Pão Doce, Rua do Cruzeiro, Leiria** (`node/13464213795`, geohash-5 `ez1dn`), chosen Sept 8; the fixtures, the seeded cell and the terminal card all name it. Still to do there: the owner's verbal OK and a dated screenshot of the listing; book the demo worker for Sat Sept 12 with a Thu Sept 10 backup.
