...
| `GET /tasks/:id/spec` | **exists** (lead, `apps/api/app/tasks/[id]/spec/route.ts`; the buyer's `claimed_*`/`source` fields are stripped) | worker-session, current claimant only → `{task_type, spec}` with `spec` = the per-type spec (`CompareTwoSpec` here). Absent from `api-contract.ts` at dispatch → `BLOCKED:` and stop; this task is optional, there is no workaround |
...
## 15. Round 2+
Merged (Sept 6, #102). Both §13 items were already closed on `main` (`GET /tasks/:id/spec` from #100; `submit`'s `proofHash` optional). The two mount points landed in the lead's follow-up PR: `/tasks` renders `UnverifiedTasks` (banner + the open rows of `GET /public/feed`, titled `<type> · <area>` since the feed carries no title) when the session probe settles without a session instead of redirecting, and the proof header links `Report task` → `/report/<id>`. The operator's phone pass (one compare-two rehearsal, one report) stays open.
