...
- `lib/area.ts`: `DEFAULT_AREA = 'ez5ku'`; `areaFromPosition(lat, lon)` = `ngeohash.encode(lat, lon, 5)`; `resolveArea(timeoutMs = 5000)` tries `getCurrentPosition` once and falls back to the default.
...
| `registerBodyExact` | with geolocation unavailable, `POST /register` body is exactly `{worker_address: <payout address>, area: 'ez5ku', task_types: 15}` |
...
- Leiria placeholders only; default area `ez5ku`.
