...
## 2. Exact scope
- `app/tasks/page.tsx` (`/tasks`): `requireVerified()`; polls `GET /tasks/list?area=&lat=&lon=` every 3 s (the path `api-contract.ts`'s `listTasks` and `apps/api/app/tasks/list/route.ts` serve; `/tasks` on the API is POST-only) (`area` from `resolveArea()`, `lat/lon` from `lastKnownPosition()` when known), paused while `document.hidden`, resumed on focus; 401 → redirect `/`. Empty state: "No open tasks near you right now — the list refreshes every 3 s."
...
- Expanded: address (the `title` — the API renders it as `<place> · <street>, <locality>`), the question by type (`verify-open`: "Is it open right now?"; `photo-of`: "Photograph the subject named in the title"; `call-confirm`: "Call and ask the template question shown after you claim"; `compare-two`: "Pick A or B against the criterion shown after you claim"), then, when the row's `brief` carries it, what the worker is actually asked — `photo-of`: `brief.subject` (+ `subject_detail`); `call-confirm`: `brief.template_question`; `compare-two`: `brief.criterion_id` as a mono tag; `verify-open` needs nothing more — proof requirements by type (`verify-open`/`photo-of`: "photo of the door + hours sign · location · timestamp"; `call-confirm`: "your answer + the time you called — self-reported, unverified"; `compare-two`: "one choice + one line"), the line "you are paid for the proof, not the answer", `Button variant="primary" size="lg" full` `CLAIM`, chip `relayed claim · gas paid by Legwork`.
...
|---|---|---|
| `GET /tasks/list?area=&lat=&lon=` | `api-contract.ts` / `docs/api.md` | worker-session → `{tasks:[{task_id, task_type, title, price_usdc, distance_m?, claim_expires_in_s?, state, seeded, brief}]}` (open + lazily-expirable); `brief` carries `place {name, street_address, locality}`, `question`, `subject`, `subject_detail`, `template_question`, `criterion_id` when the spec has them |
...
## 15. Round 2+
Round 2 (Sept 6, #95): the list must poll `GET /tasks/list` — the brief carried the pre-wave-2 path and the API's `/tasks` is POST-only; the tests passed only because T-24's mock answers both. And the `brief` the row already carries is rendered under the type-derived question (§2 amended above; §13's fallbacks were conditional on the fields being absent, and they are on `main`). `apps/miniapp/tests/tasks/**` added to the front matter.
