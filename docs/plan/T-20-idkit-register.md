...
| `TxQueue`, `FakeChain` | `@legwork/chain` (T-07) | `send({role, contract, fn, args}) → {hash}`; decoded revert names; `FakeChain.calls[]` with role + args; `failNextWith(name)` |
...
- Every chain write goes through `TxQueue` (`registerFor` as role `relayer`); the verifier key signs offchain only and never sends a transaction.
