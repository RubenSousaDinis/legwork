...
## 15. Round 2+
Merged first round (#119, `2b9f5d9`). Raised and resolved on the way: the ABI drift (lead PR #120), `TREASURY_ADDRESS` equal to the relayer against an immutable `TaskEscrow.treasury` (a distinct receive-only wallet, decided before the broadcast), and a gitleaks false positive on a renamed local (lead PR #121). The shared deployer / relayer / verifier key is the operator's choice, on record. The one reproducibility gap — §9's rehearsal needs the deployer funded on a fresh anvil — was closed in the lead's sync PR by making `--anvil` fund the roles itself. **Redeployed Sept 7** after T-32 found AbuseMark could not receive its ERC-721 (lead PR #125 and the redeploy PR): all four contracts again, one set, start block 46506271; the wrapper gained delta-based worker rows and a wait before its after-reads.
