...
## 15. Round 2+
Merged first round (#117, `f0cf184`). Rulings: `deps?` resolved at first use instead of §2's default parameter — accepted, it keeps the `not_markable` guard ahead of any boot; T-16's call shapes kept as declared overloads onto one core — accepted; `lookup_failed` surfacing as `no_identity` — accepted; the `FakeChain` record is `{fn, role, args}` — asserted as such. The switch of `hire.ts` and `POST /check` to `logScreening` / `upsertPoster` (and `rule_id: 'accept'`) was the lead's handover PR, not a round 2; that PR also made the object shape carry `claimed`. `mark_status` on `RefusalPayload` is raised and open.
