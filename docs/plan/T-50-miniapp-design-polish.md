...
Round 1 merged (#144, Sept 8). Rulings: a node carrying `data-floor="20"` renders at 20 px whatever the prototype drew (the banner's 17/15 became 20/16, the collapsed price 20); `data-floor` came off the `±n m` readout and off `[data-completed]` per §2.6, with T-47's read as the arbiter for the latter; `StatusBadge` is dropped from the phone card. The inline `var(--ink-text)` on `[data-error="claim"]` that `tests/tasks/claim.test.tsx` pinned is replaced by a class check in the lead's sync after the merge, and the allowlist entry with it. Not verified in the review: a browser render at 390 px — the operator's phone pass, once the hosted mini-app redeploys.
